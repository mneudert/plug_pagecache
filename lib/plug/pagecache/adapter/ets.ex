defmodule Plug.PageCache.Adapter.ETS do
  ...
  def init(opts) do
    auto_expire = opts[:auto_expire] || 0

    ets_tid =
      case opts[:table] do
        nil ->
          Logger.warn(
            "No ETS table configured for cache '#{opts[:name]}'." <> " Cache will be noop!"
          )

          nil

        table ->
          :ets.new(table, [:protected, :named_table, :set])
      end

    {:ok, %{auto_expire: auto_expire, ets_tid: ets_tid}}
  end
